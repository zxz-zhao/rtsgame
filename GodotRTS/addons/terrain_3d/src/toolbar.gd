...
func add_tool_button(p_params: Dictionary) -> void:
	...
	button.set_tooltip_text(translate_to_zh(p_params.get("add_text", "blank")))
	...
	if p_params.has("sub_text"):
		...
		button2.set_tooltip_text(translate_to_zh(p_params.get("sub_text", "blank")))
...
func translate_to_zh(p_text: String) -> String:
	match p_text:
		"Add Region (E)": return "添加分区 (E) - 增大地图边界"
		"Remove Region": return "移除分区 - 减小地图边界"
		"Raise (R)": return "升高地形 (R) - 抬高山脉、斜坡"
		"Lower (R)": return "降低地形 (R) - 挖深河谷、水坑"
		"Smooth (Shift)": return "平滑地形 (Shift) - 消除突起或陡峭边缘"
		"Height (H)": return "固定高度/平整 (H) - 制作平台、基地平地"
		"Slope (S)": return "创建斜坡 (S) - 绘制连接不同高度的两点斜坡"
		"Paint Texture (B)": return "绘制地表贴图 (B) - 涂刷草地、沙地、岩石"
		"Spray Texture (V)": return "喷涂混合贴图 (V)"
		"Paint Autoshader (A)": return "启用自动着色 (A) - 基于坡度自动融合地表"
		"Disable Autoshader (A)": return "禁用自动着色 (A)"
		"Paint Color (C)": return "涂抹地表颜色 (C) - 增添阴影或特定色调"
		"Remove Color (C)": return "移除地表颜色 (C)"
		"Paint Wetness (W)": return "涂抹湿度/反射率 (W) - 表现湿润地面或水坑"
		"Remove Wetness (W)": return "移除湿度/反射率 (W)"
		"Add Holes (X)": return "地表开洞 (X) - 制作地下通道/矿井入口"
		"Remove Holes (X)": return "填补地表洞口 (X)"
		"Paint Navigable Area (N)": return "绘制寻路行走区域 (N) - 控制AI寻路"
		"Remove Navigable Area (N)": return "擦除寻路行走区域 (N)"
		"Instance Meshes (I)": return "放置植被/静态物体 (I) - 批量放置树木、石头"
		"Remove Meshes (I)": return "擦除植被/静态物体 (I)"
		_: return p_text
